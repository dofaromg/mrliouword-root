...
| 憑證 | 暴露來源 | 動作 | 狀態 |
|------|----------|------|------|
| CF API token `cfut_…` | 公開證據頁明文 | Dashboard Roll | **待輪替** |
| bridge key `MrLiouWord2026` | 同上（且為弱密碼） | 換強隨機值 | **待輪替** |
| `mrl_admin_…` / `mrl_owner_…` | 同上 | app 端重簽 | **待輪替** |
| 帳號內過量 token（~22 顆，多為全帳號／無到期） | 歷史累積 | 依「還在用？權限過大？」逐顆 Roll/Delete | **清理中** |
