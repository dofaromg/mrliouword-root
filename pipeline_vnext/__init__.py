...
"""pipeline_vnext — MRL 本地檔案同步管線的可執行實作。
...
節點對應：
- python_stage `snapshot_create` → pipeline_vnext.stages.stage_clean_snapshot
- cmd `bridge_localfs`           → cli/main.py 的 bridge_localfs 指令
- runner `audit` / `rebuild` / `repair` → pipeline_vnext.runners
"""
